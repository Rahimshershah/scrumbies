...
export async function GET(
  ...
  try {
    ...
    const filesDir = join(baseUploadsDir, 'document-files')

    // Check multiple possible locations
    const possiblePaths = [
      join(filesDir, filename),
      join(process.cwd(), 'uploads', 'document-files', filename),
      join('/var/www/scrumbies', 'uploads', 'document-files', filename),
    ]
    ...
    if (!foundPath) {
      console.error(`Document file not found: ${filename}`)
