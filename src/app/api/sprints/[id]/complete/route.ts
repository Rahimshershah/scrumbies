  ...
  try {
    ...
    const sprint = await prisma.sprint.findUnique({
      ...
      include: {
        tasks: {
          where: { 
            status: { notIn: ['DONE', 'LIVE'] } // Both DONE and LIVE are completed statuses
          },
          include: {
            comments: {
              include: {
    ...
    if (action === 'close_all') {
      ...
      for (const task of openTasks) {
        ...
        await prisma.activity.create({
          data: {
            type: 'STATUS_CHANGED',
            metadata: { from: task.status, to: 'DONE', reason: 'Sprint completed' },
            taskId: task.id,
            userId: user.id,
          },
        })
      ...
      for (const task of openTasks) {
        ...
        await prisma.activity.create({
          data: {
            ...
            metadata: { from: sprint.name, to: nextSprint.name, reason: 'Sprint completed' },
            taskId: task.id,
            userId: user.id,
          },
        })
      ...
      for (const task of openTasks) {
        ...
        await prisma.activity.create({
          data: {
            type: 'STATUS_CHANGED',
            metadata: { from: originalStatus, to: 'DONE', reason: 'Sprint completed - task split' },
            taskId: task.id,
            userId: user.id,
          },
        })
